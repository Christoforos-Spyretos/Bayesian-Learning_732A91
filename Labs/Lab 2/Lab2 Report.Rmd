...
The above graph illustrates the actual temperatures (blue points) and a collection of regression curves, one for each draw from the prior. The graph provides generally good results as almost all of the curves are placed in the region of the actual temperatures. Two represent a little higher temperatures but follow the same pattern as the actual ones. Only one curve shows but results; in the very beginning has really low temperatures, but in the end, the curve is in the region of the actual temperatures. Thus, the prior hyper-parameters do not need a significant change. 
...
It is evident that 95% equal tail posterior probability intervals do not contain most of the data points. It should not contain most of the data points, because the interval illustrates uncertainty around the median value.
...
#reading data
women <- read.delim("WomenAtWork.dat",header=TRUE)
...
The approximate 95% equal tail posterior probability interval for the regression coefficient to the variable *NSmallChild* is given by the below expression.

$[E(\theta|y) - 1.96 \cdot SD(\theta|y),E(\theta|y) + 1.96 \cdot SD(\theta|y)]$
...
The above interval has a lower and an upper bound less than 0; as a result, it strengthens the assumption mentioned above that the *NSmallChild* feature plays a significant role if a woman is employed
...
prediction2 <- function(N,data,posterior_mode,posterior_covariates, nwomen){
  ...
  for (i in 1:nwomen){
    #binomial distribution 
    res[i] <- sum(rbinom(n = 7,size = 1,prob = pred))
  }
...
# 
# ggplot(pred_women2 , aes(x=pred)) +
#   geom_histogram(color = "navy", fill = "steelblue2") +
#   ggtitle("Posterior Predictive Distribution For The Number Of Women") +
