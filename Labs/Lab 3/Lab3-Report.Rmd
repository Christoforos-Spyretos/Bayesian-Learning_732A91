...
The posterior predictive density has almost the same pattern as the density of the actual values; thus it could be assumed that the the posterior's predictive values agree with the data.
...
In the above plots, the blue lines illustrate the convergence of each $\widetilde{\beta}$ sampled by the Random Walk Metropolis algorithm, and the red lines represent the actual posterior mode. More specifically, it could be assumed that the mode of all $\widetilde{\beta}$ converges to the actual mode, although it is not great in some cases.
...
The above traceplots represent the convergence of the samplers when $\phi$ equals 0.2. After approximately 150 iterations all values seems to convergence to the actual ones.
...
The above traceplots represent the convergence of the samplers when $\phi$ equals 0.95. After approximately 150 iterations all values seems to convergence to the actual ones. An interesting feature is that $\mu$ has extreme values in same cases, which is expected because of its "big" 95% credible interval.
